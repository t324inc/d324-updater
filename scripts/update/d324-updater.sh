...
DDDDDDDDDDDDD         333333333333333    222222222222222           444444444  UUUUUUUU     UUUUUUUUPPPPPPPPPPPPPPPPP
D::::::::::::DDD     3:::::::::::::::33 2:::::::::::::::22        4::::::::4  U::::::U     U::::::UP::::::::::::::::P
D:::::::::::::::DD   3::::::33333::::::32::::::222222:::::2      4:::::::::4  U::::::U     U::::::UP::::::PPPPPP:::::P
DDD:::::DDDDD:::::D  3333333     3:::::32222222     2:::::2     4::::44::::4  UU:::::U     U:::::UUPP:::::P     P:::::P
  D:::::D    D:::::D             3:::::3            2:::::2    4::::4 4::::4   U:::::U     U:::::U   P::::P     P:::::P
  D:::::D     D:::::D            3:::::3            2:::::2   4::::4  4::::4   U:::::D     D:::::U   P::::P     P:::::P
  D:::::D     D:::::D    33333333:::::3          2222::::2   4::::4   4::::4   U:::::D     D:::::U   P::::PPPPPP:::::P
  D:::::D     D:::::D    3:::::::::::3      22222::::::22   4::::444444::::444 U:::::D     D:::::U   P:::::::::::::PP
  D:::::D     D:::::D    33333333:::::3   22::::::::222     4::::::::::::::::4 U:::::D     D:::::U   P::::PPPPPPPPP
  D:::::D     D:::::D            3:::::3 2:::::22222        4444444444:::::444 U:::::D     D:::::U   P::::P
  D:::::D     D:::::D            3:::::32:::::2                       4::::4   U:::::D     D:::::U   P::::P
  D:::::D    D:::::D             3:::::32:::::2                       4::::4   U::::::U   U::::::U   P::::P
DDD:::::DDDDD:::::D  3333333     3:::::32:::::2       222222          4::::4   U:::::::UUU:::::::U PP::::::PP
D:::::::::::::::DD   3::::::33333::::::32::::::2222222:::::2        44::::::44  UU:::::::::::::UU  P::::::::P
D::::::::::::DDD     3:::::::::::::::33 2::::::::::::::::::2        4::::::::4    UU:::::::::UU    P::::::::P
DDDDDDDDDDDDD         333333333333333   22222222222222222222        4444444444      UUUUUUUUU      PPPPPPPPPP

EOF
echo "$(tput setaf 4)D324 Updater$(tput sgr 0)";
echo "";
clear_stdin;
...
backup_skipped_modules () {
  if [ -f ${BASEDIR}/vendor/t324inc/d324-updater/config/.skip-update ]; then
    mkdir -p ${BASEDIR}/update_backups/skip
    mkdir -p ${BASEDIR}/update_backups/skip/contrib
    mkdir -p ${BASEDIR}/update_backups/skip/d324
    while read p; do
      if [ -d "${BASEDIR}/${DRUPALPATH}/modules/contrib/${p}" ]; then
        cp -r ${BASEDIR}/${DRUPALPATH}/modules/contrib/${p} ${BASEDIR}/update_backups/skip/contrib/;
      fi
      if [ -d "${BASEDIR}/${DRUPALPATH}/modules/d324/${p}" ]; then
        cp -r ${BASEDIR}/${DRUPALPATH}/modules/d324/${p} ${BASEDIR}/update_backups/skip/d324/;
      fi
    done < ${BASEDIR}/vendor/t324inc/d324-updater/config/.skip-update
  fi
}
...
cleanup(){
  ...
  composer d324-version-check composer-patches;
  result="$?";
  if [ "$result" -ne 0 ]; then
  ...
  if [ -f ${BASEDIR}/vendor/t324inc/d324-updater/config/.download-before-update ]; then
    rm -rf ${BASEDIR}/vendor/t324inc/d324-updater/config/.download-before-update;
  fi
...
download_before_update(){
  if [ -f ${BASEDIR}/vendor/t324inc/d324-updater/config/.download-before-update ]; then
    ...
    done < ${BASEDIR}/vendor/t324inc/d324-updater/config/.download-before-update
  fi
}
...
copy_after_update(){
  if [ -f ${BASEDIR}/vendor/t324inc/d324-updater/config/.skip-update ]; then
    while read p; do
      if [ -d "${BASEDIR}/update_backups/skip/contrib/${p}" ]; then
        cp -r ${BASEDIR}/update_backups/skip/contrib/${p} ${BASEDIR}/${DRUPALPATH}/modules/contrib/;
      fi
      if [ -d "${BASEDIR}/update_backups/skip/d324/${p}" ]; then
        cp -r ${BASEDIR}/update_backups/skip/d324/${p} ${BASEDIR}/${DRUPALPATH}/modules/d324/;
      fi
    done < ${BASEDIR}/vendor/t324inc/d324-updater/config/.skip-update
  fi
}
...
enable_after_update(){
  if [ -f ${BASEDIR}/vendor/t324inc/d324-updater/config/.enable-after-update ]; then
    while read p; do
    ...
    done < ${BASEDIR}/vendor/t324inc/d324-updater/config/.enable-after-update
  fi
}

echo "$(tput setab 2)";
composer d324-version-check current-message;
echo "$(tput sgr 0)";
echo "$(tput setaf 2)This command will guide you to update your D324 project.$(tput sgr 0)";
echo "";
...
echo -e "$(tput setaf 2) \t 2. Cleanup and update your composer.json to prepare for D324 updates$(tput sgr 0)";
echo -e "$(tput setaf 2) \t 3. Update D324 using (composer update)$(tput sgr 0)";
...
if [ "$answer" != "${answer#[NnEe]}" ] ;then
  ...
  echo -e "$(tput setaf 2)Preparing composer.json for D324 updates...$(tput sgr 0)";
  echo -e "$(tput setaf 2)Preparing composer.json for D324 updates...$(tput sgr 0)" >> ${ERRORLOG};
  cleanup;
  composer d324-refactor-composer ${BASEDIR}/composer.new.json ${DRUPALPATH};
  result="$?";
  if [ "$result" -ne 0 ]; then
      echo -e "$(tput setab 1)$(tput setaf 7)There was an error while preparing composer.json for D324 updates. Please check ${ERRORLOG} for more information.$(tput sgr 0)";
      echo -e "$(tput setab 1)$(tput setaf 7)If you are running D324 8.x-4.x or 8.x-5.x version, make sure to update d324-project using the update command: $(tput sgr 0)";
      echo -e "$(tput setaf 2)composer require t324inc/d324-updater$(tput sgr 0)";
      exit_and_revert;
  fi
  backup_skipped_modules;
  ...
  echo -e "$(tput setaf 2)Updating D324...$(tput sgr 0)";
  echo -e "$(tput setaf 2)Updating D324...$(tput sgr 0)" >> ${ERRORLOG};
  composer update 1> >(tee -a ${ERRORLOG} >&1) 2> >(tee -a ${ERRORLOG} >&2);
  result="$?";
  if [ "$result" -ne 0 ]; then
      echo -e "$(tput setab 1)$(tput setaf 7)There was an error while updating D324 to the latest version. Please check ${ERRORLOG} for more information.$(tput sgr 0)";
      exit_and_revert;
  fi
  ...
  echo -e "$(tput setaf 2)Enabling new required modules for the latest D324 version...$(tput sgr 0)";
  echo -e "$(tput setaf 2)Enabling new required modules for the latest D324 version...$(tput sgr 0)" >> ${ERRORLOG};
  enable_after_update;
  ...
  if [ -f ${BASEDIR}/vendor/t324inc/d324-updater/config/.skip-update ]; then
    rm -rf ${BASEDIR}/vendor/t324inc/d324-updater/config/.skip-update;
  fi
  if [ -f ${BASEDIR}/vendor/t324inc/d324-updater/config/.enable-after-update ]; then
    rm -rf ${BASEDIR}/vendor/t324inc/d324-updater/config/.enable-after-update;
  fi
  ...
  composer d324-version-check next-message;
else
