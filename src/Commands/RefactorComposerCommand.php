...
use t324inc\Composer\Helpers\VersionHelper;

class RefactorComposerCommand extends BaseCommand{

  protected function configure()
  {
    $this->setName('d324-refactor-composer');
  ...
  protected function getPaths($package, $drupalPath = "docroot") {
    ...
    $paths["d324ModulesPath"] = $this->getDrupalRoot(getcwd(), $paths["rootPath"]) . "/modules/d324/";
    $paths["d324ThemesPath"] = $this->getDrupalRoot(getcwd(), $paths["rootPath"]) . "/themes/d324/";
      ...
      foreach($projectExtras["installer-paths"] as $path => $types){
        foreach($types as $type){
          if($type == "type:drupal-module"){
            ...
            if(in_array('vendor:t324inc', $types)) {
              $paths["d324ModulesPath"] = $this->getDrupalRoot(getcwd(), "") . $typePath;
              break;
            } else {
          ...
          if($type == "type:d324-theme"){
            $typePath = preg_replace('/\{\$.*\}$/', "", $path);
            if(in_array('vendor:t324inc', $types)) {
              $paths["d324ThemesPath"] = $this->getDrupalRoot(getcwd(), "") . $typePath;
              break;
            } else {
  ...
  public function array_merge_recursive_distinct(array &$array1, array &$array2, $drupalPath){
    ...
    foreach ($array2 as $key => &$value) {
        ...
        $newKey = preg_replace('/web/', $drupalPath, $newKey);
        ...
        if (is_array($value) && isset($merged[$newKey]) && is_array($merged[$newKey])) {
            ...
            $newValue = preg_replace('/web/', $drupalPath, $newValue);
  ...
  public function generate($savePath, $drupalPath) {
    ...
    $d324MetaData = [];
    $composerProjectJsonUrl = "https://raw.githubusercontent.com/t324inc/satis/gh-pages/all.json";
    ...
    if(file_exists($filename)){
      $d324Packages = JsonFile::parseJson(file_get_contents($filename), $filename);
      foreach($d324Packages as $packageName => $d324Package) {
        if($packageName == "t324inc/d324") {
          $d324MetaData = $d324Package;
        }
      }
    }

    $latestTags = VersionHelper::getLatestVersionInfo($d324MetaData);
